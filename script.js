...
(function initScrollManifesto() {
  const section = document.querySelector('.manifesto--scroll');

  if (!section) return;

  const words = [
    ...section.querySelectorAll('[data-manifesto-word]')
  ];
  ...
  function updateManifesto() {
    ...
    /*
      Sumamos algunos pasos extra para que las primeras
      palabras no aparezcan todas juntas.
    */

    const visibleCount = Math.floor(
      progress * (words.length + 4)
    );
